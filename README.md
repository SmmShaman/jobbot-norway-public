...
## 🎓 Важливі Уроки з Попередньої Сесії

**Ця секція містить критичні знання з попередньої сесії - ОБОВ'ЯЗКОВО ПРОЧИТАЙ!**

### ⚠️ Проблема #1: Нова сесія на старій гілці

**Що відбувається:**
- Нова сесія клонує репозиторій
- Git переключає на default branch (може бути `main` або стара гілка)
- Ти читаєш README.md → бачиш 32-83 рядки замість 500+
- SESSION_CONTEXT.md НЕ існує!
- database/, web-app/, worker/ папок НЕ існує!
- Починаєш працювати на СТАРОМУ коді ❌
...
# 2. Перевір довжину README
wc -l README.md
# Має бути: 500+ lines (not 32 or 83!)

# 3. Перевір наявність критичних файлів
...
### ⚠️ Проблема #2: API ключі НЕ зберігаються

**Що треба знати:**
- Кожна нова сесія НЕ має доступу до API ключів
- Ти НЕ можеш "запам'ятати" SUPABASE_SERVICE_KEY з минулої сесії
- НЕ можеш "запам'ятати" gcp-key.json

**Що робити:**
```
# Якщо потрібен доступ до Supabase:
"Для роботи з Supabase мені потрібен SUPABASE_SERVICE_KEY.
Зайди: https://supabase.com/dashboard/project/ptrmidlhfdbybxmyovtm/settings/api
Скопіюй 'service_role' key і дай мені."

# Якщо потрібен деплой на Cloud Run:
"Для деплою на Cloud Run:
Варіант 1: Виконай `gcloud auth login` (якщо gcloud налаштований)
Варіант 2: Дай мені gcp-key.json файл
Варіант 3: Скажи і я дам інструкції як задеплоїти вручну"
```

### ⚠️ Проблема #3: Плутанина браузерний vs термінальний Claude

**Браузерний Claude (ти зараз):**
```
ТИ (браузер) МОЖЕШ:
✅ git commit, git push
✅ Редагувати код (web-app, backend, worker)
✅ Створювати SQL скрипти
✅ Деплоїти на Netlify (git push)
✅ Деплоїти на Cloud Run (з ключами)
✅ Працювати з Supabase API (з ключами)

ТИ (браузер) НЕ МОЖЕШ:
❌ Запустити Worker на ПК користувача
❌ Доступ до localhost:8000 (Skyvern)
❌ Запустити docker-compose
❌ Створити .env файли на ПК користувача
❌ Виконати pip install на ПК користувача
```

**Термінальний Claude (на ПК користувача):**
```
Користувач запускає:
cd ~/jobbot-norway-public
...
✅ Запустити Worker (python3 worker/worker.py)
✅ Налаштувати Skyvern
✅ Запустити docker-compose
✅ Створити .env файли
✅ pip install -r requirements.txt
✅ Перевірити localhost:8000

Флаг --dangerously-skip-permissions = автономна робота БЕЗ переспитувань!
```

**Коли що використовувати:**
```
Якщо треба змінити КОД → Браузерний Claude (ти)
Якщо треба ЗАПУСТИТИ щось локально → Термінальний Claude (користувач)

Приклад розподілу:
- Ти (браузер): "Я оновив worker.py, зробив commit і push"
- Користувач на ПК: запускає термінальний Claude який робить "git pull && python3 worker/worker.py"
```

### ⚠️ Проблема #4: Netlify vs Cloud Run - різні підходи
...
→ Деплоїть на https://jobbot-norway.netlify.app
→ НЕ ПОТРІБНІ КЛЮЧІ! ✅
```

**Google Cloud Run (ВРУЧНУ або з КЛЮЧАМИ):**
```bash
# Варіант 1: Якщо gcloud налаштований
gcloud auth list  # Перевірити
gcloud run deploy jobbot-backend --source ./backend --region us-central1

# Варіант 2: Якщо є gcp-key.json
export GOOGLE_APPLICATION_CREDENTIALS=/path/to/gcp-key.json
gcloud auth activate-service-account --key-file=$GOOGLE_APPLICATION_CREDENTIALS
gcloud run deploy ...

# Варіант 3: Попросити користувача
"Я оновив backend код і зробив commit.
Будь ласка, задеплой на Cloud Run:
cd backend
gcloud run deploy jobbot-backend --source . --region us-central1"
```

### ⚠️ Проблема #5: Різні гілки мають різний код

**Існуючі гілки:**
```
main → СТАРИЙ код (або порожній redirect)
claude/netlify-ui-011CUqJXNw4wkoYPis8TAkxF → СТАРА робоча гілка
claude/add-metadata-master-scheduler-011CUqJXNw4wkoYPis8TAkxF → АКТУАЛЬНА гілка ✅
```
...
# Подивитися останні коміти АКТУАЛЬНОЇ гілки
git log origin/claude/add-metadata-master-scheduler-011CUqJXNw4wkoYPis8TAkxF --oneline -5
...
97d683c - Add complete authentication guide
```

### ⚠️ Проблема #6: Суперечлива інформація в старих файлах

**Якщо ти на СТАРІЙ гілці, можеш побачити:**
- workflows/*.json (N8N workflows - більше не використовуються!)
- src/complete_ai_application_system.py (старий підхід)
- NEXT_STEPS.md (застарілі інструкції)

**НЕ плутайся!** Актуальна архітектура:
```
web-app/ → React frontend (Netlify)
backend/ → FastAPI (Cloud Run)
worker/ → Python Worker (локально на ПК)
database/ → SQL schemas (Supabase)

workflows/ та src/ - це СТАРІ файли, НЕ використовуються!
```
